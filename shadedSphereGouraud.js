...
async function loadGLB(url) {
    ...
    return {
        ...
        indexType: primitive.indices.value instanceof Uint32Array ? gl.UNSIGNED_INT : gl.UNSIGNED_SHORT,
        indexCount: primitive.indices.value.length
    };
}
/**
 ...
 *
 * This function was sourced through a combination of Google Gemini queries, stack overflow, and simple
 * google searches for parsing Gaussian Splat model data. A library was not used to perform this import because it was
...
window.onload = async function init() {
    ...
    const splatData = await parseSplatPLY("/model.ply");
    const meshData = await loadGLB("/model.glb");
    ...
    colorTexLoc = gl.getUniformLocation(lightingProgram, "u_ColorTex");
    posTexLoc = gl.getUniformLocation(lightingProgram, "u_PosTex");

    makeSb(); // build the skybox object
    defaultTex(); // bind a default texture to the skybox
...
function render() {
    ...
    modelViewMatrix = mult(modelViewMatrix, rotateX(modelRotationX));
    modelViewMatrix = mult(modelViewMatrix, rotateY(modelRotationY));
    modelViewMatrix = mult(modelViewMatrix, rotateZ(modelRotationZ));
    ...
    if (meshToggle){
        ...
        var skyboxMatrix = scalem(50.0, 50.0, 50.0); // scale around scene (could be small ? TEST THIS STILL)
